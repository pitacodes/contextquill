...
function keychainToken(memberUrn) {
  ...
  try {
    return execFileSync(
      "security",
      ["find-generic-password", "-s", "contextquill-linkedin-access-token", "-a", memberUrn, "-w"],
...
export class ContextQuillStore {
  ...
  async connectionStatus() {
    ...
    return {
      connected: Boolean(credentials.memberUrn && credentials.accessToken),
      member_urn_configured: Boolean(credentials.memberUrn),
      token_available: Boolean(credentials.accessToken),
      credential_source: credentials.source,
      linkedin_version: credentials.linkedinVersion,
  ...
  buildLinkedInPayload(draft, authorUrn, media = null) {
    ...
    if (media) payload.content = { media };
    return payload;
  }
  ...
  async uploadImage(attachment, credentials) {
    ...
    const headers = {
      ...
      "Linkedin-Version": credentials.linkedinVersion,
    ...
    const initialized = await this.linkedinFetch(
      "https://api.linkedin.com/rest/images?action=initializeUpload",
      {
        method: "POST",
        headers,
        body: JSON.stringify({ initializeUploadRequest: { owner: credentials.memberUrn } }),
      },
