...
import {
  ...
  LINKEDIN_CLIENT_ID,
  LINKEDIN_SCOPES,
} from "./linkedin-oauth.mjs";
import { deleteLinkedInToken, getLinkedInToken } from "./secure-credentials.mjs";
...
export class ContextQuillStore {
  ...
  async configureProfile(input = {}) {
    ...
    const next = {
      ...
      linkedin_scopes: input.linkedin_scopes ? cleanArray(input.linkedin_scopes) : current.linkedin_scopes || [],
  ...
  async linkedInCredentials(profile) {
    ...
    const accessToken = process.env.LINKEDIN_ACCESS_TOKEN || getLinkedInToken(memberUrn);
    return {
      memberUrn,
      accessToken,
  ...
  async connectionStatus() {
    ...
    return {
      ...
      setup_command: "npm run authorize-linkedin",
      setup_url: null,
      oauth_mode: "hosted_authorization_code",
      oauth_service: process.env.CONTEXTQUILL_OAUTH_BASE_URL || DEFAULT_CONTEXTQUILL_OAUTH_BASE_URL || null,
      client_id: LINKEDIN_CLIENT_ID,
      granted_scopes: cleanArray(profile.linkedin_scopes),
  ...
  async verifyLinkedInConnection() {
    ...
    if (!credentials.memberUrn || !credentials.accessToken) {
      throw new ContextQuillError(
        "LinkedIn is not connected. Run `npm run authorize-linkedin` or use the connect_linkedin tool.",
  ...
  async disconnectLinkedIn() {
    const profile = await this.getProfile();
    const memberUrn = normalizeMemberUrn(profile.linkedin_member_urn);
    const credentialDeleted = deleteLinkedInToken(memberUrn);
    const next = {
      ...profile,
      linkedin_member_urn: "",
    ...
    await this.set("profile", next);
    await this.audit("linkedin.disconnected", { member_urn: memberUrn || null, credential_deleted: credentialDeleted });
    return { disconnected: true, credential_deleted: credentialDeleted };
  }
